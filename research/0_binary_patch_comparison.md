...
| JB-25 | B     | `patch_vm_map_protect`                | `_vm_map_protect`                                                                                    | Skip upstream write-downgrade gate. Shape A (26.1–26.4) active; **Shape B (26.5) disabled 2026-07-05** — widened the `~VM_PROT_WRITE` COW strip (vm_map.c:6202) instead of the RWX gate (vm_map.c:5997), breaking COW and crashing the debugger (SPTM `VIOLATION_ILLEGAL_MAP`). Retired on 26.5+: SPTM code-mod (debugger + Substrate tweaks) uses write-then-flip via `vm_protect(VM_PROT_COPY)` → `XNU_USER_DEBUG`, so no RWX patch is needed. **OBSERVED (2026-09-22, cloudOS 26.4 `c0ecdb4b…`, jb):** Shape A did not match on this base either, so with Shape B disabled the patch applies nothing and logs `[-] vm_map_protect write-downgrade gate not found` — which `make test_fw_patches` is built to fail on. Left as-is deliberately: whether a 26.4 base still needs the RWX bypass is a semantics question, not a pattern-matching one, and the boot validated without it. Note the source disagrees with itself about the covered range — `KernelJBPatchVmProtect.swift` headers say "Shape A (26.1 / 26.3)" at lines 9 and 75 but "Shape A stays for 26.1-26.4" at line 69, and this row says 26.1–26.4. One of the three is wrong; resolving that is the prerequisite for deciding whether the `[-]` should become a gated skip. |    Y/N     |
